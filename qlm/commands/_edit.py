...
def edit(file: str = Argument(..., help="The path to the file you want to edit relative to remote or local repo root.")
    ...
    if is_offline():
        ...
        if not os.path.exists(path_to_file_for_editing):
            print(Panel(f"[bold red1] The file [yellow]{file}[/yellow] doesn't exist :confused:"))
            raise Exit()
        elif os.path.isdir(path_to_file_for_editing):
        ...
        tmp: NamedTemporaryFile = NamedTemporaryFile()
