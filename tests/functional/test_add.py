...
def test_add_no_local(runner):
    ...
    assert f"Oh no 😟 You haven't yet set a value for the key local_repo" in result.stdout
...
def test_add_offline(runner, local_repo_with_file, remote_repo):
    ...
    assert get_config(key="offline_files_to_add") == [{"repo_filepath": "humphrey.md",
                                                       "local_filepath": os.path.join(local_repo_with_file,
                                                                                      "humphrey.md"),
                                                       "remote": remote_repo}]
