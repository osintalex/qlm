...
def test_get_overwrite_disallowed(runner, online_mode, fake_pat, remote_repo, mock_download_file_for_get):
...
def test_get_success(runner, online_mode, fake_pat, remote_repo, mock_download_file_for_get):
...
@pytest.mark.parametrize("rename, directory", [
    ("new file.md", TemporaryDirectory()),
    ("", TemporaryDirectory()),
    ("new file.md", ""),
],)
def test_get_success_filepath_and_name_combinations(rename, directory, runner, online_mode, fake_pat, remote_repo, mock_download_file_for_get):
    ...
    if rename and directory:
        result = runner.invoke(app, ["get", filename, "--rename", rename, "--directory", chosen_directory])
